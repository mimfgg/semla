...
The query language is agnostic of the storage vendor and should remain the same if you were to migrate from one database vendor to another.

It is fully extensible but comes with those maven modules:
...
* [semla-jdbi](/semla-jdbi): generic SQL databases support using [jdbi](http://jdbi.org/)
...
Semla comes with a plugin to generate typed EntityManagers `io.semla.persistence.TypedEntityManager` with generated type safe methods for all the properties of your types.

Given that you annotate a `User` class with `io.semla.persistence.annotations.Managed` 

And that you add this plugin to your project:
```xml
<plugin>
    <groupId>io.semla</groupId>
...
You can even write a Datasource for your favorite DB flavor if it's not already supported!
...
By default, the following datasources are included in the core library:
 ...
 - ShardedDatasource: sharding on primary key and can rebalance automatically if a shard is added.
...
Semla packs its own dependency injection framework which can be configured during the configuration with:
...
- `io.semla.datasource.DatasourceFactory`: creating and holding all the `io.semla.datasource.Datasource<T>` instances (1 per type)
- `io.semla.persistence.EntityManagerFactory`: creating and holding all the generic `io.semla.persistence.EntityManager<T>` intances
- `io.semla.persistence.TypedEntityManagerFactory`: creating and holding all the `io.semla.persistence.TypedEntityManager` implementations.
...
However, if you have run the maven plugin to generate your TypeEntityManager classes, those 2 TypeEntityManager should be available:
```java
...
the behaviour should be identical as the second one is mostly a wrapper around the first one.

*The methods on the generic EntityManager are the same but they use a String parameter in place of the field name, or the enum values.*
...
```java
 Group defaultGroup = groupManager.newGroup("default").persist();
 User user = userManager.newUser("bob").group(defaultGroup).persist();
```
...
You can either modify an entity you already have:
```java
 user.name = "tom";
...
Relations can be traversed in both directions. For example, we can fetch all the users in the same group than bob:
```java
...
Semla comes with its own simple query language mapping the query executed.
...
It is mostly used by the tests and for debugging purpose, as it allows for reparsing the query in the logs. 
...
if `@StrictIndices` is added to the class, then only the primary key and the explicitely indexed properties will be queriable.
...
Semla includes both a Json and a Yaml serializer/deserizalizer. Available as a singleton through the `Json` and `Yaml` classes, 
...
While being not as configurable as Jackson, it should be enough for most projects. Current options are:
...
The Yaml parser supports references and anchors and also including sub files through the `!include` tag:
...
Regular methods have to be explicitely annotated to be serialized/deserialized.
...
This will make a `GraphQL` and a `GraphQLProvider` instances available in your injector.
The `GraphQL` instance will be configured with the base schema for all your entities, 
so you should already be able to access your database right away.
