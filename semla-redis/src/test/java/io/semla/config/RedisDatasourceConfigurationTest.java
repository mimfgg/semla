...
public class RedisDatasourceConfigurationTest {
    ...
    static {
        Types.registerSubTypes(RedisDatasourceConfiguration.class);
    }

    @Test
    public void create() {
        EntityModel<Player> model = EntityModel.of(Player.class);
        RedisDatasourceConfiguration config = RedisDatasource.configure()
            .withHost("localhost")
            .withPort(1234)
    ...
    @Test
    public void parse() {
        DatasourceConfiguration configuration = Yaml.read("type: redis", DatasourceConfiguration.class);
        assertThat(configuration).isInstanceOf(RedisDatasourceConfiguration.class);
    }
}