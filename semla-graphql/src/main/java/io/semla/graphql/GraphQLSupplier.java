...
public class GraphQLSupplier implements Supplier<GraphQL> {
    ...
    @Inject
    public GraphQLSupplier(EntityManagerFactory factory,
    ...
    public GraphQLSupplier addQuery(String query, BiFunction<Injector, DataFetchingEnvironment, ?> handler) {
        additionalQueries.put(query, handler);
        graphQL = null;
        return this;
    }

    public GraphQLSupplier addMutation(String query, BiFunction<Injector, DataFetchingEnvironment, ?> handler) {
        additionalMutations.put(query, handler);
        graphQL = null;
        return this;
    }

    public GraphQLSupplier addTypes(String... types) {
    ...
    private String getSchema(List<EntityModel<?>> models) {
        ...
        if (!models.isEmpty() || !additionalMutations.isEmpty()) {
            builder.append("\ntype Mutation {\n");
            models.forEach(model -> {
    ...
    protected <T> void addFields(EntityModel<T> model, String name, boolean isUpdate, StringBuilder builder) {
        builder.append(model.members().stream()
                .filter(member -> !member.annotation(Transient.class).isPresent())
                .map(member -> {
