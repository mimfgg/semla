Feature: OneToOne relationships
  ...
  Scenario: the "mappedBy" parameter is mandatory on the reverse @OneToOne annotation with JoinTable
    ...
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Parent\.child or .+Child\.parent needs to be set
      """
    Then an exception InvalidPersitenceAnnotationException is thrown when the model of {{run}}.Child is generated
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Child\.parent or .+Parent\.child needs to be set
      """
  ...
  Scenario: the "mappedBy" parameter on the reverse @OneToOne annotation must point to a valid property
    ...
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Child\.parent defines a non existent member 'kid' on class .+\.Parent
      """
  ...
  Scenario: the "mappedBy" parameter on the reverse @OneToOne annotation must point to a field annotated with @OneToOne
    ...
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Child\.parent defines a member 'name' on class .+\.Parent that is not annotated with @OneToOne
      """
  ...
  Scenario: the "mappedBy" parameter cannot be set on both sides of the @OneToOne
    ...
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Child\.parent defines a member 'child' on class .+\.Parent that also defines a member 'parent'. Only one of the two class can own this relationship!
      """
  ...
  Scenario: the "mappedBy" parameter on the @OneToOne annotation must point to a valid property
    ...
    And exception.message is equal to:
      """
      ?e @OneToOne\.mappedBy on .+Parent\.child defines a non existent member 'daddy' on class .+\.Child
      """
