...
public class KeyValueRelationsTest {
    ...
    public void fetchTheParentIncludesOnlyTheChildrenReferences() {
        ...
        Assertions.assertThat(author1.books).isNotNull();
        Assertions.assertThat(author1.books.stream().map(book -> book.id).collect(Collectors.toList())).isEqualTo(Lists.of(1, 2, 3));
    ...
    private void assertThatauthor1BooksAreFetched(Author author1) {
        Assertions.assertThat(author1.books).isNotNull();
        Assertions.assertThat(author1.books.stream().map(book -> book.id).collect(Collectors.toList())).isEqualTo(Lists.of(1, 2, 3));
        Assertions.assertThat(author1.books.stream().noneMatch(EntityModel::isReference)).isTrue();
    }
    ...
    public void cascadePersist() {
        ...
        Assertions.assertThat(issac_asimov.books).isNotNull();
        Assertions.assertThat(issac_asimov.books).size().isEqualTo(1);
    ...
    public void cascadeMerge() {
        ...
        Assertions.assertThat(author2.books).isNotNull();
        Assertions.assertThat(author2.books).size().isEqualTo(1);
        Assertions.assertThat(author2.books.get(0).author).isNotNull();
        Assertions.assertThat(author2.books.get(0).author).isEqualTo(author2);
        Assertions.assertThat(author2.books.get(0).author.name).isEqualTo("author2");
        ...
        Assertions.assertThat(books.get(4).get().name).isEqualTo("newNameForBook1");
    }
    ...
    public void cascadeRemove() {
        authors.newAuthor("author2")
            ...
            .create(author -> author.books());
        Assertions.assertThat(books.get(4).get().name).isEqualTo("book1");
        authors.delete(2, author -> author.books());
        Assertions.assertThat(authors.get(2).isPresent()).isFalse();
        Assertions.assertThat(books.get(4).isPresent()).isFalse();

        authors.delete(Lists.of(1L), author -> author.books());
        Assertions.assertThat(authors.get(1).isPresent()).isFalse();
