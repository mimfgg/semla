...
public class JoinTables {
    ...
    public static String generateName(Member<?> member, EntityModel<?> parentModel) {
        return Stream.<Supplier<String>>of(
            () -> parentModel.getType().getCanonicalName() + Types.optionalTypeArgumentOf(member.getGenericType())
                .map(Class::getSimpleName).orElse(member.getType().getSimpleName()),  // ParentChild
            () -> parentModel.getType().getCanonicalName() + "_" + Types.optionalTypeArgumentOf(member.getGenericType())
