...
public class TypedEntityManager<K, T, GetType, CreateType, SetterType, PatchType, SelectType, PredicateTypes, IncludesType> extends AbstractEntityManager<T> {

    private final Class<GetType> getType = typeArgumentOf(this.getClass().getGenericSuperclass(), 2);
    private final Class<CreateType> createType = typeArgumentOf(this.getClass().getGenericSuperclass(), 3);
    private final Class<SetterType> setterInterface = typeArgumentOf(this.getClass().getGenericSuperclass(), 4);
    private final Class<PatchType> patchInterface = typeArgumentOf(this.getClass().getGenericSuperclass(), 5);
    private final Class<SelectType> selectType = typeArgumentOf(this.getClass().getGenericSuperclass(), 6);
    private final Class<PredicateTypes> predicatesInterface = typeArgumentOf(this.getClass().getGenericSuperclass(), 7);
    private final Class<IncludesType> includesType = typeArgumentOf(this.getClass().getGenericSuperclass(), 8);
    ...
    protected CreateType newInstance() {
        Create<T> create = new Create<>(newContext(), model().newInstance());
    ...
    private static <T> boolean isManaged(Relation<T, ?> relation) {
        if (Types.isAssignableTo(relation.member().getType(), Collection.class)) {
            return typeArgumentOf(relation.member().getGenericType(), 0).isAnnotationPresent(Managed.class);
        }
    ...
    private static <T> String getTypeName(Relation<T, ?> relation) {
        if (Types.isAssignableTo(relation.member().getType(), Collection.class)) {
            return getManagerCanonicalName(typeArgumentOf(relation.member().getGenericType(), 0));
        }
