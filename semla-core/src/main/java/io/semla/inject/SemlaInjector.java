...
public class SemlaInjector implements Injector {
        ...
        if (!factoryCache.computeIfAbsent(type, t -> new LinkedHashMap<>()).containsKey(annotationsSignature)) {
            ...
            if (candidates.isEmpty()) {
                ...
                if (clazz.isAnnotationPresent(Singleton.class)) {
                    candidates.add(new TypedProviderFactory<>(type, io.semla.util.Singleton.lazy(() -> createNewInstance(clazz)), ANY));
                } else {
    ...
    public static SemlaInjector create(Throwables.UnaryOperator<Binder>... bindings) {
        ...
        binder.scopedFactories.put(Singleton.class, (type, provider, predicates) ->
            new TypedProviderFactory<>(type, io.semla.util.Singleton.lazy(provider::get), predicates)
        );
        ...
        binder.multiBindings.forEach((t, b) -> b.forEach(((filter, suppliers) -> {
            binder.injector.factories.add(new Factory<Object>() {
                ...
                public boolean appliesTo(Type type, Annotation[] annotations) {
                    Optional<Class<?>> argument = Types.optionalTypeArgumentOf(type);
    ...
    protected static class SemlaBinder implements Binder {
        ...
        @Override
        public Binder register(Class<? extends Annotation> scope, TriFunction<Type, Provider<?>, AnnotationMatcher, Factory<?>> factoryBuilder) {
        ...
        protected static abstract class SemlaFilteredBinding<SelfType> implements FilteredBinding<SelfType> {
            ...
            public SelfType named(String name) {
                return annotatedWith(Annotations.named(name));
            }
        ...
        protected class SemlaBinding<E> extends SemlaFilteredBinding<Binding<E>> implements Binding<E> {
            ...
            public SemlaBinder to(Class<? extends E> clazz) {
                return toScopedProvider(() -> injector.createNewInstance(clazz));
            }

            @Override
            public SemlaBinder to(E instance) {
                return toProvider(() -> instance);
            }

            @Override
            public Binder toConstructor(Constructor<? extends E> constructor) {
                return toScopedProvider(() -> unchecked(() -> constructor.newInstance(injector.getInjectedParametersFor(constructor))));
            }

            @Override
            public SemlaBinder toProvider(Class<? extends Provider<? extends E>> providerClass) {
                return toScopedProvider(injector.getInstance(providerClass));
            }

            protected SemlaBinder toScopedProvider(Provider<? extends E> provider) {
                if (scope != null) {
                    TriFunction<Type, Provider<?>, AnnotationMatcher, Factory<?>> factoryBuilder = scopedFactories.get(scope);
                    if (factoryBuilder == null) {
                        throw new IllegalArgumentException("no scopedFactory for scope: " + scope.getCanonicalName());
                    }
                    injector.addFactory(factoryBuilder.apply(type, provider, annotationMatcher));
            ...
            public SemlaBinder toProvider(Provider<? extends E> provider) {
                injector.addFactory(new TypedProviderFactory<>(type, provider, annotationMatcher));
                return SemlaBinder.this;
            }
        }
