...
import static io.semla.model.FruitManager.Sort.name;
...
public class TypedEntityManagerTest {
    ...
    public void typedManagerTest() {
        ...
        assertThat(banana.genus).isNotNull();
        assertThat(banana.genus.name).isEqualTo("musa");

        banana = fruits.where().name().is("banana").first(fruit -> fruit.genus(genus -> genus.family(), genus -> genus.fruits())).get();

        assertThat(banana.genus).isNotNull();
        assertThat(banana.genus.family).isNotNull();
        assertThat(banana.genus.family.name).isEqualTo("Musaceae");
        assertThat(banana.genus.fruits.size()).isEqualTo(1);
        assertThat(banana.genus.fruits.get(0).id).isEqualTo(banana.id);

        fruits.newFruit().name("apple").price(2).genus(malus).create();

        assertThat(fruits.orderedBy(name().desc()).first().get().name).isEqualTo("banana");
        assertThat(fruits.orderedBy(name().asc()).first().get().name).isEqualTo("apple");
        assertThat(fruits.orderedBy(name().asc()).startAt(0).limitTo(1).list().size()).isEqualTo(1);
