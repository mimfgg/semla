Feature: a json serializer/deserializer
  ...
  Scenario: a simple entity in a really malformed json
    When the following json gets parsed as a io.semla.model.Fruit:
    """
    {"id":1,     "name":
    "banana",
  ...
  Scenario: a java bean as a json
    When the following json gets parsed as a io.semla.model.Bean:
    """
    {"id":1,"name":"test"}
    """
    Then it gets pretty serialized as this json:
  ...
  Scenario: polymorphic serialization/deserialization
    ...
    """
    When the following json gets parsed as a io.semla.serialization.json.Datasources:
    """
    {"datasources":[
      {
        "type": "in-memory"
